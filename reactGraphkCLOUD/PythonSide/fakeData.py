...
class EthereumHandler:
    ...
    def injectFakeData(self): 
      ...
      [
        ...
        for x in range(0, len(coordinates)):
            ...
            .buildTransaction({'chainId': 3, 'gas': 800000, 'gasPrice': self.w3.toWei('200', 'gwei'), 'nonce': (self.w3.eth.getTransactionCount(self.account._address)+counter)})
            nonce = nonce + 1
    ...
    def enrollDrum(self):
        for x in range(0, 3):
            ...
            1, 1, 1, 1, 1)\
        .buildTransaction({'chainId': 3, 'gas': 800000, 'gasPrice': self.w3.toWei('20', 'gwei'), 'nonce': (self.w3.eth.getTransactionCount(self.account._address) + x)})
    ...
    def passSensorData(self, _sensorId, _time, _latitude, _longitude, _accX, _accY, _accZ, _temp, _humi, _radio, _end, nonce):
        ...
        KCLOUD_txn = self.KCLOUD.functions.passSensorData(\
            dataId, sensorId, time, latitude, longitude, accX, \
            accY, accZ, temp, humi, radio)\
        .buildTransaction({'chainId': 3, 'gas': 800000, 'gasPrice': self.w3.toWei('20', 'gwei'), 'nonce': (self.w3.eth.getTransactionCount(self.account._address) + nonce)})
